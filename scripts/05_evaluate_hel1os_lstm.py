...
def evaluate_on_dataset(model, dataset_path, device, dataset_name, feature_cols, max_batches=200):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=["Quiet", "Flare"], yticklabels=["Quiet", "Flare"])
    plt.title(f"{dataset_name} Confusion Matrix")
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.tight_layout()
    
    cm_path = os.path.join(project_root, f"{dataset_name.lower()}_confusion_matrix.png")
    plt.savefig(cm_path)
    print(f"Saved {dataset_name} Confusion Matrix to: {cm_path}")
    
def main():
    model_path = os.path.join(project_root, "models", "hel1os_lstm_model.pth")
    ...
    # Evaluate on ~400,000 sequences of HEL1OS
    evaluate_on_dataset(model, hel1os_path, device, "HEL1OS", feature_cols=['CTR_NORM', 'STAT_ERR'], max_batches=200)
