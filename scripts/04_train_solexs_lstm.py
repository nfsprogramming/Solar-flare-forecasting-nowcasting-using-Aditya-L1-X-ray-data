...
def train_solexs_lstm():
    project_root = r"d:\NFS's Projects\Solar flare forecasting nowcasting using Aditya-L1 X-ray data"
    solexs_path = os.path.join(project_root, "datasets", "processed", "solexs_dataset.parquet")
    model_save_path = os.path.join(project_root, "models", "solexs_lstm_model.pth")
    ...
    print("Initializing Parquet Sequence Data Streamer for SoLEXS...")
    dataset = ParquetSequenceDataset(solexs_path, window_size=60, forecast_horizon=10, feature_cols=['COUNTS', 'COUNTS_NORM'])
    ...
    # 3. Initialize Model
    # Input size is 2 (COUNTS, COUNTS_NORM), hidden size 64
    ...
    print(f"SoLEXS Model saved to {model_save_path}")
